...
:::caution
`Configure()` is called once at startup, when `MapAxisEndpoints()` reads each endpoint's routing
and metadata. At that point the endpoint instance is created without running its constructor, so
any fields normally set by constructor injection are not available yet. `Configure()` must only
call methods on the `IEndpointConfiguration` argument — it must not read `this` fields or call
other instance members. `HandleAsync()`, by contrast, always runs on a fully constructed instance
