...
Call `RequireAuthorization()` with no arguments to require an authenticated user without restricting to a specific role or policy.

```csharp
public class ProfileEndpoint : IEndpoint<Response<ProfileResponse>>
{
    public void Configure(IEndpointConfiguration config)
    {
        config.Get("/profile").RequireAuthorization();
    }
    // ...
}
```
...
Authorization set on a group applies to all endpoints in that group. An endpoint's own
authorization setting fully replaces the group's rather than being combined with it; an
endpoint that declares no setting of its own inherits the group's setting unchanged.
